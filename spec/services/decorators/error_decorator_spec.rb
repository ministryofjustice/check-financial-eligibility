...
module Decorators
  RSpec.describe ErrorDecorator do
    describe "#as_json" do
      subject(:decorator) { described_class.new(param).as_json }

      context "String" do
        let(:param) { "This is an error" }

        it "puts the string in the response struct" do
          expect(decorator).to eq(errors: ["This is an error"], success: false)
        end
      end
      ...
      context "CheckFinancialEligibilityError" do
        ...
        it "puts the error message in the response struct" do
          expect(decorator).to eq(errors: ["my message"], success: false)
        end
      end
      ...
      context "Other exceptions" do
        ...
        it "generates a response struct with the correct keys" do
          expect(decorator.keys).to eq(%i[errors success])
        end
