module WorkflowService
  class CapitalCollator < BaseWorkflowService
    ...
    def liquid_capital
      @liquid_capital ||= LiquidCapitalAssessment.call(assessment)
    end

    def non_liquid_capital
      @non_liquid_capital ||= NonLiquidCapitalAssessment.call(assessment)
    end

    def property
      @property ||= PropertyAssessment.call(assessment)
    end

    def vehicles
      @vehicles ||= VehicleAssessment.call(assessment)
    end
    ...
    def pensioner_capital_disregard
      @pensioner_capital_disregard ||= WorkflowService::PensionerCapitalDisregard.new(assessment).value
    end
