...
def get_value_from_response(response, version, attribute)
  section_path = get_mapping version, attribute

  # Drill down into the JSON and extract the value out. Works with hash and array structures.
  value = ""
  section_path.split(".").each do |bit|
    if response.is_a?(Array)
      bit = bit.to_i
    end

    if response[bit].nil?
      raise "Expected to have key '#{bit}' in '#{response}' using attribute '#{attribute}' with path '#{section_path}'"
    end
